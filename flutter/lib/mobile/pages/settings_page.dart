...
const url = 'https://1support.co.kr/';
...
class _SettingsState extends State<SettingsPage> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
      ...
      children: [
        if (bind.isCustomClient())
          Align(
            ...
            // child: loadPowered(context), // Hide "Powered by RustDesk"
      ...
      enhancementsTiles.insert(
          ...
          SettingsTile.switchTile(
              ...
              title: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(translate('Keep 원서포트 background service')),
    ...
    final settings = Obx(() => SettingsList(
      sections: [
        customClientSection,
        if (!bind.isDisableAccount())
          SettingsSection(
            ...
            tiles: [
              SettingsTile(
                title: Text(gFFI.userModel.isLogin
                    ? '${translate('Logout')} (${gFFI.userModel.userNameC.value})'
                    : translate('Login')),
              ...
              if (!gFFI.userModel.isLogin)
                SettingsTile(
                  ...
                  onPressed: (context) {
                    launchUrlString('https://1support.co.kr/register');
                  },
                ),
            ],
          ),
        ...
        if (isAndroid && !incomingOnly)
          SettingsSection(title: Text(translate('Recording')), tiles: [
            SettingsTile.switchTile(
              ...
              onToggle: isOptionFixed(kOptionAllowAutoRecordIncoming)
                  ...
                  : (v) async {
                      await bind.mainSetOption(
                          ...
                          value: v ? 'Y' : 'N');
                      final newValue = await bind.mainGetOption(
                              key: kOptionAllowAutoRecordIncoming) ==
                          'Y';
                      setState(() {
              ...
              onToggle: isOptionFixed(kOptionAllowAutoRecordOutgoing)
                  ...
                  : (v) async {
                      await bind.mainSetLocalOption(
                          ...
                          value: v ? 'Y' : 'N');
                      final newValue = await bind.mainGetLocalOption(
                              key: kOptionAllowAutoRecordOutgoing) ==
                          'Y';
                      setState(() {
        ...
        if (isAndroid && !outgoingOnly && !hideSecuritySettings)
          SettingsSection(
            ...
            tiles: [
              ...
              SettingsTile.switchTile(
                ...
                onToggle: isOptionFixed(kOptionDirectServer)
                    ...
                    : (v) async {
                        await bind.mainSetOption(
                            key: kOptionDirectServer, value: v ? 'Y' : 'N');
                        final newValue =
                            await bind.mainGetOption(key: kOptionDirectServer) ==
                                'Y';
                        setState(() {
                          _enableDirectIPAccess = newValue;
                        });
                      },
              ),
            ],
...
void showAbout(OverlayDialogManager dialogManager) {
  dialogManager.show((setState, close, context) {
    return CustomAlertDialog(
      title: Text(translate('About 원서포트')),
